...
Watches GitHub resources for state changes — pull requests (CI, reviews, comments, merge conflicts), commits (CI status), branches (new commits), and tags (new tags, optionally ones including a specific commit).
...
Use the `/watch-tag` skill to monitor a repository's tags — useful when waiting for a release tag, or for the tag that includes a specific merged commit.
